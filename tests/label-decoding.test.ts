...
it.each(['qr', 'code128'] as const)('decodes the actual printed %s symbol back to its saved variant code', async symbology => {
  ...
  const html = renderLabelDocument({symbology,width:symbology==='qr'?40:100,height:30,items:[{id:'v1',copies:1}]},[{id:'v1',sku:code,barcode:code,title:'Gold / 7',inventoryQuantity:12,product:{id:'p1',title:'Rings 004'}}])
