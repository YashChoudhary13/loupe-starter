...
for (const symbology of ['qr', 'code128'] as const) {
  const html = renderLabelDocument({ symbology, width: symbology === 'qr' ? 40 : 70, height: symbology === 'qr' ? 25 : 30, items: variants.map(v => ({ id: v.id, copies: 1 })) }, variants)
...
writeFileSync(resolve(output, 'labels-screen.html'), `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Loupe Labels — sample screen</title><style>${css}</style></head><body style="background:#ededed;font-family:Arial,sans-serif"><p style="padding:16px;font-size:13px">Sample screen — real Shopify search and printing require a signed-in Loupe session after deployment.</p>${renderToStaticMarkup(<LabelsScreen query="NK1333" variants={variants} />)}</body></html>`)
