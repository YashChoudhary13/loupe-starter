-- 2026-08-21 evening: the Identify screen is in the repo but Railway has not
-- deployed it, so a photograph parked in 'identifying' had nothing to release
-- it and the owner's uploads sat queued. Until the deploy is confirmed, new
-- photographs land in 'discovered' (enhancement starts at once) AND still get
-- a match event, so the matcher keeps working in the background and nothing
-- is lost. Re-enable the gate by re-applying the D110 bodies in a new migration
-- once /identify answers on production.
...
comment on function public.discover_intake_file(text, text, text, bigint, text, text) is
  'Idempotently inserts a Drive file before work, writes intake.discovered, permanently rejects unsupported MIME types or files over 50 MB (50,000,000 bytes) with attempts=1, and requests a catalogue match — non-blocking until the Identify screen is deployed (D110).';

comment on function public.finalize_raw_image_upload(uuid, text, integer, integer, text, text, text) is
  'Turns a verified browser upload into queue work with an optional per-photograph prompt binding (D103) and a non-blocking catalogue match (D110, gate off until the Identify screen is deployed).';
