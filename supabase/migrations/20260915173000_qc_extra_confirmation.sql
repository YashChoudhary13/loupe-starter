-- Record extras during continuous scanning; complete only after each extra is ticked removed.
...
begin
  ...
  if p_action not in ('sync','scan','complete','reset','undo','clear_extra') then raise exception 'Unknown QC action.'; end if;
