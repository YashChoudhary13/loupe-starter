-- One round trip per scan: the RPC returns the recent audit and the open shortages of the current checklist
-- alongside the session, so the server no longer follows every command with two more reads (D133).
-- Same qc_command signature as 20260918090000; only the return value gains `events` and `shortages`.
create or replace function public.qc_view(p_session_id uuid, p_generation integer) returns jsonb
language sql
stable
security invoker
set search_path = ''
as $$
  select jsonb_build_object(
    'events', coalesce((
      select jsonb_agg(jsonb_build_object('id',x.id,'action',x.action,'outcome',x.outcome,'message',x.message,'code',x.code,'line_id',x.line_id,
        'variant_id',x.variant_id,'actor_id',x.actor_id,'actor_name',x.actor_name,'created_at',x.created_at,'generation',x.generation,'undo_of',x.undo_of)
        order by x.created_at desc, x.id desc)
      from (select * from public.qc_events where session_id=p_session_id order by created_at desc, id desc limit 40) x
    ), '[]'::jsonb),
    'shortages', coalesce((
      select jsonb_agg(jsonb_build_object('id',x.id,'ref',x.ref,'session_id',x.session_id,'event_id',x.event_id,'order_id',x.order_id,'order_name',x.order_name,
        'generation',x.generation,'line_id',x.line_id,'variant_id',x.variant_id,'sku',x.sku,'title',x.title,'variant_title',x.variant_title,'quantity',x.quantity,
        'reason',x.reason,'reported_by',x.reported_by,'reported_at',x.reported_at,'resolved_at',x.resolved_at,'resolved_by',x.resolved_by,'resolution',x.resolution,'resolution_note',x.resolution_note)
        order by x.ref)
      from public.qc_shortages x where x.session_id=p_session_id and x.generation=p_generation and x.resolved_at is null
    ), '[]'::jsonb)
  );
$$;
revoke all on function public.qc_view(uuid,integer) from public, anon, authenticated;
grant execute on function public.qc_view(uuid,integer) to service_role;
...
begin
  ...
  if p_action='sync' then return jsonb_build_object('session',to_jsonb(s)) || public.qc_view(s.id,s.generation); end if;
  ...
  if found then
    ...
    return jsonb_build_object('session',to_jsonb(s),'event',to_jsonb(e),'replayed',true) || public.qc_view(s.id,s.generation);
  end if;
  ...
  return jsonb_build_object('session',to_jsonb(s),'event',to_jsonb(e),'replayed',false) || public.qc_view(s.id,s.generation);
end;
$$;
