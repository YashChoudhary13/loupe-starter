-- Whole-order packing QC. Shopify remains the authority for remaining quantities.
-- Browser clients have no table/RPC access; the authenticated server supplies snapshots.
create table public.qc_sessions (
  ...
  status text not null default 'checking' check (status in ('checking','passed','stale')),
...
create table public.qc_events (
  ...
  undo_of uuid references public.qc_events(id),
  detail jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  unique (session_id, request_id),
  unique (session_id, undo_of)
);
create index qc_events_session_created_idx on public.qc_events(session_id, created_at desc, id desc);
create index qc_sessions_status_updated_idx on public.qc_sessions(status, updated_at desc);
alter table public.qc_sessions enable row level security;
alter table public.qc_events enable row level security;
revoke all on public.qc_sessions, public.qc_events from public, anon, authenticated;
grant select, insert, update on public.qc_sessions to service_role;
grant select, insert on public.qc_events to service_role;

create or replace function public.qc_command(
  p_shop_domain text,
  p_order_id text,
  p_actor_id uuid,
  p_action text,
...
begin
  ...
  if p_action not in ('sync','scan','complete','reset','undo') then raise exception 'Unknown QC action.'; end if;
  ...
  if p_action in ('reset','undo') and (p_reason is null or length(trim(p_reason)) not between 3 and 240) then
  ...
  new_session := found;
  -- The order row serializes every scan, reset, undo and completion, across devices.
  ...
  cmd := jsonb_build_object('action',p_action,'actor',p_actor_id,'code',p_code,'version',p_expected_version,'undo',p_undo_event_id,'reason',p_reason,'generation',p_expected_generation);
  ...
  if p_action = 'scan' and p_expected_generation is distinct from s.generation then
    ...
    if p_rejection is not null then
      ...
      outcome := 'extra'; message := 'QC has already passed. This extra scan was not added.';
    else
      ...
      if line is null then
        if exists (select 1 from jsonb_array_elements(s.snapshot->'lines') v where v->>'variantId'=p_variant_id) then
          outcome := 'extra'; message := 'All required units of this variant are already checked. Remove the extra item.';
        else outcome := 'wrong'; message := 'This variant is not required by this order. Set it aside.'; end if;
      else
        target_line := line->>'id';
    ...
    if target.id is null or target.outcome <> 'accepted' or target.generation <> s.generation or target.actor_id <> p_actor_id
    ...
    if jsonb_array_length(s.snapshot->'lines')=0 or exists (select 1 from jsonb_array_elements(s.snapshot->'lines') v
      where coalesce((s.counts->>(v->>'id'))::integer,0) <> (v->>'required')::integer) then
      outcome := 'incomplete'; message := 'Items are still missing. Scan every remaining unit before completing QC.';
    else
      ...
      outcome := 'passed'; message := 'QC passed for all remaining shipping units. Shopify fulfillment is unchanged.';
    end if;
  end if;
  ...
  values(s.id,p_request_id,cmd,s.generation,p_action,outcome,message,p_code,target_line,p_variant_id,p_actor_id,actor,undo_id,detail)
  returning * into e;
  return jsonb_build_object('session',to_jsonb(s),'event',to_jsonb(e),'replayed',false);
end;
$$;
revoke all on function public.qc_command(text,text,uuid,text,jsonb,text,timestamptz,uuid,text,text,text,integer,uuid,text,integer) from public, anon, authenticated;
grant execute on function public.qc_command(text,text,uuid,text,jsonb,text,timestamptz,uuid,text,text,text,integer,uuid,text,integer) to service_role;
comment on table public.qc_sessions is 'Packing counts for the whole remaining shipping order; never changes Shopify fulfillment.';
comment on table public.qc_events is 'Append-only QC audit and per-request deduplication ledger. No customer information is stored.';
