...
1. **Shopify scopes for Dispatch** (owner, Dev Dashboard) — not needed for the deploy; Dispatch shows its "cannot read or fulfil orders yet" message until then. When added and re-approved, verify with a read-only `currentAppInstallation { accessScopes { handle } }` query, then make the first live Dispatch pushes the 2026-09-21 PROGRESS entry lists.
2. **DNS audit.** Every `*.qimati-eng.site` record points at the owner's own infrastructure; remove dangling CNAMEs (Railway/Vercel leftovers) and third-party names. The apex, `qc.` and `ship.` already resolve.
3. **Server.** Install the branch's `deploy/loupe.nginx.conf` by hand (`sudo install -m 644 … /etc/nginx/sites-available/loupe`), `sudo nginx -t` with no "conflicting server name" warning, reload (`sudo systemctl reload nginx`); `https://qc.qimati-eng.site/health`, `https://ship.qimati-eng.site/health` and `https://qimati-eng.site/health` must return Loupe's health page (the old code serves `/health` on any host); then `certbot --expand` for the three names (optional while Cloudflare is non-strict, done anyway): `sudo certbot certonly --webroot -w /var/www/html --cert-name loupe.qimati-eng.site --expand -d loupe.qimati-eng.site -d qimati-eng.site -d qc.qimati-eng.site -d ship.qimati-eng.site`, then `sudo systemctl reload nginx`.
4. **Migrations.** `20260921100000_dispatch.sql` then `20260923100000_home_probe_state.sql` via `scripts/apply-migration.ts` with the production env mirror: `npx tsx scripts/apply-migration.ts 20260921100000_dispatch.sql <env file> output/platform/dispatch-migration.json` then `npx tsx scripts/apply-migration.ts 20260923100000_home_probe_state.sql <env file> output/platform/home-migration.json`. Confirm all three tables exist and `anon` cannot read them.
5. **Rotate `AUTH_SESSION_SECRET`** (`openssl rand -hex 32`) in `~/loupe/shared/.env` and `.env.railway`, no restart — safe before the deploy because old code simply signs everyone out; operators sign in once.
6. **Deploy.** Push one no-ff merge commit to `main` (first parent = the current `main`, so a later `git revert -m 1` undoes the platform in one commit); watch `~/loupe/shared/autodeploy.log`. `AUTH_BASE_URL` stays on Loupe.
7. **Live checks.** All lights green or honestly amber; the Awaiting QC tile shows a number (proves the `ordersCount` search terms and the 300-id lookup); ask "which orders are awaiting QC?" and "is Shopify healthy?" and see the `home.chat_turn` row with a cost; sign in on `qc.` (returns to `qc.`), open `ship.` with no second sign-in; scan one QC label and print one label on `qc.`; sign out on `loupe.` and confirm the old cookie is gone.
8. **Rollback, if needed.** Instant: `bash ~/loupe/repo/scripts/rollback.sh b7f5220` on the server. Permanent: `git revert -m 1 <deploy merge>` and push.
9. **Later.** Move `AUTH_BASE_URL` to the Home origin (Google redirect URI first, then env and restart, then re-check); the n8n key and `N8N_*`/`HOME_N8N_WORKFLOWS` together; the two bot webhooks; the memory vault (`/Users/yash/Desktop/Qimati Memory/systems/loupe.md`, `operations/credential-register.md` with key names only, one `log.md` entry with the evidence from step 7).
