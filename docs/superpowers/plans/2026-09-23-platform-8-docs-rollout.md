...
```
Expected: every test file PASS; `home schema proof: 7 checks passed`; `dispatch schema proof: 9 checks passed`; typecheck clean; lint shows exactly the 5 baseline errors in `scripts/tmp-promote-worn.ts`, `src/components/live/LiveActivity.tsx`, `tests/app-shell-render.test.ts` and nothing else; `next build` compiles and lists `/home`, `/api/home/chat`, `/api/home/action` and the proxy; every file under 500 lines.
