...
### D124 — Sparse colour–size identities and durable order QC (2026-09-15)

The owner confirmed different sizes per colour and explicitly approved implementing and deploying the complete system. This supersedes D70's one-dimension restriction. Keep existing single modes and add `colour_size`: option_value is colour, size_value is the second dimension, stock belongs to each actual pair. The editor never manufactures unavailable cross-product rows. ProductSet sends native Color plus Size, deduplicates option values and keeps existing Shopify variant IDs by the full option identity. The old parent-stock trigger intentionally used max(stock); retain that legacy behavior for single modes and sum combination rows. New-draft codes activate in a separate migration after the compatible server is live.

Existing stock preparation is an operator-reviewed per-product SKU/Barcode mutation by variant ID, not a general republish. Preserve distinct existing barcodes; reject conflicting identities or stale previews; audit before and after. Legacy reconciliation accepts typed variant SKUs within the original parent family. No automated full-catalogue rename or physical relabeling is implied by deploying the feature.

QC covers the complete remaining shipping order, explicitly including other locations. Resolve exact codes globally to one variant ID, count by order line and require current fulfillable quantities. An invoker RPC locked on the order session serializes actions, deduplicates request UUIDs and fences scans by checklist generation after reset. Active operator checks, RLS with no browser grants, bounded routes and same-origin writes reuse Loupe's access model. All accepted/rejected/undo/reset events remain durable. Shopify edits invalidate progress; QC completion does not fulfill orders. No serial tracking: the checker must scan each pouch once and move it.
...
### D128 — Scanner-first QC with tone and image feedback, accepted shortages, 30-day history and a WhatsApp `missing` command (2026-09-18)

After the first real QC run the owner reported: the count only ticked 4/7 → 5/7 with no sound and no picture of what was just checked; the camera stayed on although the team scans with a 2D scanner gun; the list showed payment-pending orders; past checks were invisible; and a unit the team does not have at all had no path except leaving the order unfinished.

Camera now starts closed and is a deliberate per-browser choice (`localStorage`), so the scanner gun is the default. Stray keystrokes are pulled back into the code field. Every accepted scan plays a synthesized rising tone (Web Audio, no assets, created on the first user gesture), vibrates on phones, and shows a feedback card with the line's Shopify CDN thumbnail, title, option and `n / required`; rejections, extras and wrong items play a low buzz; a pass plays a chime. Line thumbnails come from `LineItem.image` on every fresh order read and are display only: the fingerprint ignores them, so existing sessions are not invalidated and old snapshots without images render from the fresh read.

The open-order list is filtered to `financial_status:paid OR partially_paid OR partially_refunded` because Qimati is prepaid with no COD; a searched order number is always shown so a specific order can be opened. A "Checked in the last 30 days" list is derived from `qc_events.outcome='passed'` — the append-only audit — so it survives fulfillment and later edits; it opens a read-only record (`?view=history`) that never reads Shopify or calls the RPC, because the live checklist would otherwise mark a fulfilled order stale and clear its pass.
...
Refinements from the task reviews the same day: store deletes are conditional on `staged`/`failed` rows and a parcel row is removed only once it is empty, so a push in flight can never be discarded; clearing a number, or absorbing a lone parcel while grouping, writes `dispatch.unstaged`; a failed Loupe write after a confirmed fulfilment is reported on the row, never thrown; the screen's push loop (`push-loop.ts`) stops at the first unanswered call and marks the remaining parcels not attempted; saving never locks a field (the scanner's Enter moves focus to the next row) and Push waits for saves in flight, so the confirm sheet always shows the number that will be sent. From the whole-branch review: **a pushed parcel is frozen** — once `pushed_at` is set the number and carrier can no longer be edited, only discarded (`stageTracking`'s parcel `update` is conditional on `pushed_at is null`), and `markPushed` re-asserts the carrier and number the push actually sent; the confirm sheet's carrier, number and order ids travel with the push and are checked against the row before anything is read from Shopify. Known ceiling: the parcel `update` in `stageTracking` is still read-then-act, so an edit racing a push can be accepted and is then superseded by the sent number at the end of that push — its `dispatch.staged` event is the trace (see the `ponytail:` comment in `store.ts`); the upgrade path is a `pushing_since` lock column on `dispatch_parcels`, if such an edit should be refused outright instead.

### D136 — One app, four faces by hostname; one sign-in on `.qimati-eng.site` (2026-09-23)

Owner: a home dashboard at `qimati-eng.site`, QC and Fulfilment as their own apps, Loupe trimmed to listing, more apps later. Built as **one codebase, one deploy, one login, one database, one Shopify app** wearing four faces. `src/lib/faces/faces.ts` maps `qimati-eng.site` → Home (`/home`), `loupe.` → Loupe (Console, Upload, Identify, Restock, Tracking, Prompts, Models, Workflows), `qc.` → Order QC (`/qc`, `/labels`), `ship.` → Fulfilment (`/dispatch`). `src/proxy.ts` (Next 16's name for middleware) sets `x-face` from the Host header and answers a screen that belongs to another face with a 307 to that face's host, same path and query; `/` goes to the face's first screen; `/api/*`, `/login`, `/health` and assets are served everywhere; an unknown host is Loupe in production, a dev machine is unrestricted unless `FACE_DEV` names a face. The shell reads the header for its menu (the face's own screens plus an Apps switcher to the other three hosts) and `<html data-face>` selects one of four token sets in `globals.css` on the same components — Loupe unchanged, Home warm white / orange / deep green, QC pink / cream, Fulfilment teal / sand. Sign-in: the session, handshake and denied cookies carry `domain=.qimati-eng.site` when `AUTH_BASE_URL` is https on that domain, which the Loupe base satisfies as well as the Home one; Google keeps one redirect URI, on whichever host `AUTH_BASE_URL` names; the start route records the face whose host began the sign-in and the callback returns there; sign-out clears the domain cookie and the pre-platform host-only one. Found while planning: the QC scan and both Labels routes accepted only the `AUTH_BASE_URL` origin and would have refused every scan on `qc.`, so they now accept every face origin (`isOwnOrigin`); `scripts/apply-migration.ts` accepts both the Loupe and the Home origin; R2's CORS origin stays `https://loupe.qimati-eng.site` (uploads happen on Loupe), which is also what `scripts/configure-r2-cors.ts` falls back to; to name it explicitly the flag has to pass through npm: `npm run r2:cors -- --origin https://loupe.qimati-eng.site`. Rejected: one deployment per app (nothing needs isolation yet); per-user app access (a later addition).

Refinements from the task reviews the same day: production redirects are always absolute — Next builds `request.url` from its bind address, so a relative redirect would point at `127.0.0.1`; `isFace` checks membership with `Object.hasOwn`, so a prototype key such as `constructor` is never read as a face; `FACE_DEV` only ever applies outside production, so it can never override production's Loupe fallback for an unmatched host; the domain cookie is also sent to every other `*.qimati-eng.site` host, today `packaging.`, `linkedin.`, `print.` and `demand.qimati-eng.site` (the Hub). Corrected by the final review: HttpOnly only stops page scripts from reading it; the session cookie is still a 12-hour bearer credential for an app that publishes to and fulfils orders in the live store, and any host under the domain receives it. The real control is that every name under the domain is served by the owner's own infrastructure, with no dangling or third-party records, checked by a DNS audit at rollout. A pre-platform host-only session on `loupe.` survives a sign-out issued from another host for up to 12 h; rotating `AUTH_SESSION_SECRET` at the cutover ends every such session, so the rotation is an explicit rollout step. It is safe before the deploy, because old code simply signs everyone out, and operators sign in once.

Sign-in base at rollout (final review, 2026-09-23): `AUTH_BASE_URL` stays `https://loupe.qimati-eng.site` for now, so every face signs in through the existing Loupe Google callback and no Google console change is needed. The new code works with either base: `cookieDomain()` gives `.qimati-eng.site` for the Loupe base too, `isOwnOrigin` accepts every face, and `apply-migration.ts` accepts both. This order deliberately differs from spec §9 step 4, which set the Home base before the deploy. Pre-platform code accepts QC scans and label prints only from the `AUTH_BASE_URL` origin and keeps its handshake cookie host-only on `loupe.`, so switching the base before the deploy breaks QC scans, label printing and sign-in whenever the old release restarts with the new env: a crash, a reboot, another push, a failed build, or `deploy.sh`'s own rollback. Moving it to the Home origin is a later step: add `https://qimati-eng.site/api/auth/google/callback` in the Google console first, then edit `~/loupe/shared/.env` (and `.env.railway`) and `sudo systemctl restart loupe`, then re-check sign-in and sign-out. After that switch, rolling back to any pre-platform release needs `AUTH_BASE_URL` reverted first.
...
Refinements from the task reviews the same day: every number in `computeNumbers` is bounded at 10 s (`NUMBERS_TIMEOUT_MS`) and every probe and probe-store call at 5 s (`PROBE_TIMEOUT_MS`); an unknown probe state — a failed `store.load()` — is treated as no previous state and never counted as a change, so a database blip cannot erase or falsify the "since" record; `ReadOnlyShopify` is a branded type (`readonly readOnly: true`) so a raw `ShopifyClient` cannot stand in for it by accident; `list_orders`'s `awaiting_qc` filter checks the newest 250 open orders and appends a note when that page is full, since the dashboard's own number covers 300; `dispatch_summary` counts a failed order once, from the open list only; a confirm token is only ever signed and read with the real 64-hex `AUTH_SESSION_SECRET`, never the bot's shared webhook secret; a calendar date is round-tripped through `Date` so an impossible date like 31 February is refused rather than silently normalised; an expired nonce is refused before it is ever checked against the used set, so it can never be spent; the webhook poster sends with `redirect: 'error'`, so `X-Loupe-Secret` can never follow a redirect to another host; a non-2xx webhook response is logged with `console.warn` on the server and never returned to the browser; the output budget is enforced for the whole turn, not per model call — up to 4 000 completion tokens across every call in one turn; a 200 response with no usable choice becomes a `type: 'error'` event, not an answer with a clean usage line; a turn that throws still writes its `home.chat_turn` row with `ok: false` and whatever usage the failed attempt had already accumulated; both chat and action routes read the request body through a byte-bounded reader before `JSON.parse` ever sees it; the red health light's `#b3261e` is the one place a third colour appears on a Loupe screen — the spec's own green/amber/red, an explicit exception to D9, with a `--red` token left as a later polish item; and the conversation lives in `ChatPanel`'s React state only, with no browser storage and nothing kept server-side between turns.
