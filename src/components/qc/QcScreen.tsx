...
export function QcScreen({ initialView }: { initialView: QcView }) {
  ...
  return <section className="h-full overflow-auto px-3 py-3 md:px-8 md:py-5">
    ...
    <div className="sticky top-0 z-10 mt-3 rounded-card bg-white p-3 shadow-sm md:mt-4 md:p-4">
      {/* Two columns from md: the scan field and its messages on the left, the last-scan card and camera on the right,
          so the sticky block stays about one card tall on a 10-inch screen instead of stacking everything. */}
