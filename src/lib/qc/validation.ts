...
export function parseQcCommand(value: unknown): QcCommand {
  ...
  if (!['scan', 'complete', 'reset', 'undo', 'clear_extra', 'short'].includes(String(input.action))) throw new Error('Unknown QC action.')
  ...
  if (command.action === 'short') {
    if (typeof input.lineId !== 'string' || !/^gid:\/\/shopify\/LineItem\/[1-9]\d{0,22}$/.test(input.lineId)) throw new Error('Choose an order line to mark short.')
    command.lineId = input.lineId
  }
  if (command.action === 'reset' || command.action === 'undo' || command.action === 'short') {
