  ...
  const data = await client.graphql<{ orders: { nodes: QcOrderSummary[]; pageInfo: PageInfo } }>(`
    query LoupeQcOrders($query: String!, $after: String) {
      orders(first: 30, after: $after, query: $query, sortKey: CREATED_AT, reverse: true) {
        nodes { id name createdAt updatedAt displayFulfillmentStatus }
        pageInfo { hasNextPage endCursor }
      }
    }`, { query: `status:open (fulfillment_status:unfulfilled OR fulfillment_status:partial)${term ? ` name:${JSON.stringify(term.replace(/^#/, ''))}` : ''}`, after })
        ...
        query LoupeQcOrder($id: ID!, $after: String) {
          order(id: $id) {
            ...
            lineItems(first: 100, after: $after) {
              nodes { id title variantTitle sku requiresShipping fulfillableQuantity variant { id sku barcode title } }
              pageInfo { hasNextPage endCursor }
            }
          }
        }`, { id: gid, after })
